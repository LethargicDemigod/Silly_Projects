...
# ╔═╡ bf0f5285-efd9-489c-859d-8ce00aec2aee
begin
	using PlutoUI
	using GLMakie
	...
	shoelace_area(points) = 0.5 * abs(sum(points[i][1]*points[i+1][2] - points[i+1][1]*points[i][2] for i in 1:4))
	
	function compute_quad(angle_deg)
	    A = Point2f(0, 0)
	    B = A + Point2f(a, 0)
	    θ = deg2rad(angle_deg)
	    C = B + Point2f(b*cos(θ), b*sin(θ))
	
	    for φ in range(0, 2π, length=720)
	        D = A + Point2f(d*cos(φ), d*sin(φ))
	        if isapprox(norm(D - C), c; atol=1.0)
	            return (A, B, C, D)
	        end
	    end
	    return nothing
...
# ╔═╡ 473ad617-19dd-4bc1-b46c-616647ba94bb
@bind angle_deg PlutoUI.Slider(0:1:150, show_value=true)


# ╔═╡ 02f2c697-d935-460f-909a-79a89d3c1a4d
begin
	points = compute_quad(angle_deg)
	area = points === nothing ? missing : shoelace_area((points..., points[1]))
	area_text = points === nothing ? "No valid quad" : "Area: $(round(area; digits=2)) units²"
	
	fig = Figure(size=(400, 400))
	ax = Axis(fig[1, 1], aspect=DataAspect(), title="Quadrilateral Shape")
	
	if points !== nothing
	    xs = [p[1] for p in (points..., points[1])]
	    ys = [p[2] for p in (points..., points[1])]
...
# ╔═╡ 37f80c06-5630-4586-bfc8-59115a72abc5
area_text
...
manifest_format = "2.0"
project_hash = "03fdaeaa5619e04bf8bf416142e5fdc4974b1e13"
